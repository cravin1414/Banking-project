...
class CGBankApp:
    """Streamlit application for CGBank with all button key issues fixed"""
    ...
    def _render_dashboard(self):
        """Render the dashboard page with unique button keys"""
        ...
        with cols[0]:
            if st.button("💸 Transfer Money", 
                        key="dashboard_transfer_btn", 
                        use_container_width=True):
                st.session_state.page = "transfer"
                st.rerun()
        with cols[1]:
            if st.button("💰 Pay Bills", 
                        key="dashboard_bills_btn", 
                        use_container_width=True):
                st.session_state.page = "bills"
                st.rerun()
        with cols[2]:
            if st.button("📊 Transactions", 
                        key="dashboard_transactions_btn", 
                        use_container_width=True):
                st.session_state.page = "transactions"
                st.rerun()
        with cols[3]:
            if st.button("🤖 Chat with Rexa", 
                        key="dashboard_rexa_btn", 
                        use_container_width=True):
    ...
    def _render_popup_bot(self):
        """Render the popup bot interface with unique button keys"""
        if st.button("💬", key="popup_toggle_btn", help="Chat with Rexa"):
    ...
    def _render_sidebar(self):
        """Render the sidebar navigation with unique button keys"""
        with st.sidebar:
            ...
            if st.session_state.logged_in:
                ...
                if st.button("🏠 Dashboard", 
                           key="sidebar_dashboard_btn", 
                           use_container_width=True):
                    st.session_state.page = "dashboard"
                    st.rerun()
                if st.button("📊 Transactions", 
                           key="sidebar_transactions_btn", 
                           use_container_width=True):
                    st.session_state.page = "transactions"
                    st.rerun()
                if st.button("💸 Transfer",  
                           key="sidebar_transfer_btn", 
                           use_container_width=True):
                    st.session_state.page = "transfer"
                    st.rerun()
                if st.button("💰 Bills", 
                           key="sidebar_bills_btn", 
                           use_container_width=True):
                    st.session_state.page = "bills"
                    st.rerun()
                if st.button("🤖 Rexa", 
                           key="sidebar_rexa_btn", 
                           use_container_width=True):
                    st.session_state.page = "rexa"
                    st.rerun()
    ...
    def _render_transactions_page(self):
        """Render the transactions page with unique keys for buttons"""
        st.markdown("### 📋 Recent Transactions")
        
        if not st.session_state.transactions:
            st.session_state.transactions = CGBankDatabase.get_user_transactions(st.session_state.current_user)
        
        for i, txn in enumerate(st.session_state.transactions[:10]):
    ...
    def _render_transfer_page(self):
        """Render the fund transfer page with unique keys"""
        ...
        with col2:
            ...
            for i, recipient in enumerate(recent_recipients):
                if st.button(f"📤 {recipient}", 
                            key=f"recipient_btn_{i}",
                            use_container_width=True):
                    st.info(f"Selected: {recipient}")

    def _render_bills_page(self):
        """Render the bills payment page with unique keys"""
        ...
        with col1:
            ...
            for i, bill in enumerate(bills):
                ...
                if st.button(f"Pay ₹{bill['amount']:,.2f}", 
                           key=f"pay_bill_btn_{i}",
                           use_container_width=True):
                    try:
